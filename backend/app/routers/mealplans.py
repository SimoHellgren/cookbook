...
def get(mealplan_id: int, db: Session = Depends(get_db)):
...
def create(mealplan: MealplanCreate, db: Session = Depends(get_db)):
    return crud.mealplan.create(db, mealplan)


@router.put("/{mealplan_id}", response_model=Mealplan, status_code=status.HTTP_200_OK)
def update(mealplan_id: int, mealplan: MealplanUpdate, db: Session = Depends(get_db)):
    db_mealplan = crud.mealplan.get(db, mealplan_id)

    if not db_mealplan:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND)

    return crud.recipe.update(db, db_obj=db_mealplan, obj_in=mealplan)
