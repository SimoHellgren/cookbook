...
from backend.app.schemas.recipe_ingredient import RecipeIngredient, RecipeIngredientUpdate
...
@router.put("/{recipe_id}:{ingredient_id}", response_model=RecipeIngredient, status_code=status.HTTP_200_OK)
def put(recipe_id: int, ingredient_id: int, obj_in: RecipeIngredientUpdate, db: Session = Depends(get_db)) -> Any:
