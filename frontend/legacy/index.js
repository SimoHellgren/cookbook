...
let api = (function () {
  ...
  const endpoint = (path) => {
    return {
      ...
      delete: (id) => fetch(APIURL + path + '/' + id, { method: 'DELETE' }).then((r) => r.json()),
    };
  };
  ...
  return {
    recipes: {
      ...
      ingredients: {
        add: (recipe_id, data) =>
          fetch(`${APIURL}/recipes/${recipe_id}/ingredients`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
    ...
    recipe_ingredients: {
      ...
      put: (recipe_id, ingredient_id, data) =>
        fetch(`${APIURL}/recipe_ingredients/${recipe_id}:${ingredient_id}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      put_many: (data) =>
        fetch(`${APIURL}/recipe_ingredients/bulk`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
          },
...
const TagGrid = (tags) => {
  ...
  let elements = tags.sort().map((tag) => {
...
const RecipeSearch = () => {
  ...
  $sidebar.append(p, form, TagGrid([...alltags]));
...
const MealCard = (mealplan) => {
  ...
  statedropdown.append(...['open', 'bought', 'done'].map((s) => new Option(s, s, false, false)));
...
const MealplanFilter = () => {
  ...
  form.append(start_label, start, end_label, end, done_label, donecheckbox);
  return form;
};
...
const CreateMealpanForm = () => {
  ...
  form.onsubmit = (ev) => {
    ...
    let plans = dates
      .map((date) =>
        meals.map(([name, servings], position) => ({
          date,
          name,
          servings,
...
const CreateMealplans = () => {
  ...
  let [modal, overlay, closefunc] = ModalOverlay(
    'create-mealplans-modal',
    'Create Mealplans',
    form,
  );
...
const AddRecipe = () => {
  ...
  form.onsubmit = async (ev) => {
    ...
    if (state.recipes.find((r) => r.name === recipe_data.name))
      alert(`Recipe with name "${recipe_data.name}" already exists`);

    //missing ingredients
    const missing = ingredients.filter(
      (a) => !state.ingredients.map((b) => b.name).includes(a.name),
    );

    //create recipe and missing ingredients
    const [recipe, _] = await Promise.all([
      api.recipes.post(recipe_data),
      Promise.all(missing.map(api.ingredients.post)).then(
        (res) => (state.ingredients = state.ingredients.concat(...res)),
      ),
    ]);
...
const EditRecipe = (recipe) => {
  ...
  form.onsubmit = async (ev) => {
    ...
    let delete_ris = state_ingredients.filter(
      (ri) => !recipeingredients.map((i) => i.ingredient).includes(ri.ingredient.name),
    );
      ...
      .then(() =>
        ...
        delete_ris.forEach((ri) => {
          ...
          api.recipe_ingredients
            .delete(recipe.id, ingredient_id)
            .then(
              (data) =>
...
const RecipeView = (recipe) => {
  ...
  let [scale_label, scale_input] = Input(
    { type: 'number', step: 'any', value: recipe.servings },
    'servings',
  );
  ...
  deletedialog.append(dialogtitle, deleteform);
  ...
  container.append(header, method, edit_modal, edit_overlay);
...
const RecipesPage = () => {
  return [RecipeSearch(), RecipeGrid(state.recipes), ...AddRecipe()];
};
...
const Comment = (comment) => {
  ...
  let metadata_text = `${comment.author}, ${created.toLocaleDateString(
    'fi',
  )} ${created.toLocaleTimeString('fi')} UTC`;

  if (comment.created !== comment.updated) {
    metadata_text += ` (edited ${updated.toLocaleDateString('fi')} ${updated.toLocaleTimeString(
      'fi',
    )} UTC)`;
  }
  ...
  let [replymodal] = ModalOverlay(
    'reply-comment-modal',
    `Reply to "${msg_preview}..."`,
    AddComentForm(comment.id),
  );
  replybutton.onclick = () => {
...
const RecipePage = () => {
  ...
  return [IngredientList(ingredients), RecipeView(recipe), CommentSection(comments)];
};
  ...
  save.onclick = () => {
    ...
    container.querySelectorAll('.mealcard').forEach((card) => {
      ...
      api.mealplans
        ...
        .then(
          (data) =>
            (state.mealplans = state.mealplans.map((mp) => (mp.id === data.id ? data : mp))),
        );
    });
  };
...
api.recipe_ingredients
  ...
  .then(
    (data) =>
      (state.recipe_ingredients = data.sort(
        (a, b) => a.recipe_id - b.recipe_id || a.position - b.position,
      )),
  );
...
api.comments.get().then((data) => (state.comments = data));
